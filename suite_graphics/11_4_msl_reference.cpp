...
namespace this_file
{
    ...
    class my_app : public motor::application::app
    {
        ...
        motor::graphics::msl_object_mtr_t msl_obj_a ;
        motor::graphics::msl_object_mtr_t msl_obj_b ;
        motor::graphics::msl_object_mtr_t msl_obj_c ;

        // used for transfering the configs to the backend.
...

            {
                ...
                // load shader libs first
                {
                    ...
                    motor::vector< motor::io::location_t > locations = { 
                        motor::io::location_t( "shaders.11_4_multi_config.msl" ) } ;
                ...
                // render shader uses config a
                {
                    motor::graphics::msl_object_t mslo("my_render_msl_a") ;

                    motor::string_t shd ;
                    auto const res =  db.load( motor::io::location_t( "shaders.11_4_use_config_0.msl" ) ).wait_for_operation( [&] ( char_cptr_t data, size_t const sib, motor::io::result const ) 
                    { 
                    ...
                    msl_obj_a = motor::shared( std::move( mslo ) ) ;
                    reconfigs.emplace_back( msl_obj_a ) ;
                }

                // render shader uses config b
                {
                    motor::graphics::msl_object_t mslo("my_render_msl_b") ;

                    motor::string_t shd ;
                    auto const res =  db.load( motor::io::location_t( "shaders.11_4_use_config_1.msl" ) ).wait_for_operation( [&] ( char_cptr_t data, size_t const sib, motor::io::result const ) 
                    { 
                        shd = motor::string_t( data, sib ) ;
                    } ) ;
                    ...
                    msl_obj_b = motor::shared( std::move( mslo ) ) ;
                    reconfigs.emplace_back( msl_obj_b ) ;
                }

                // render shader uses config c
                {
                    motor::graphics::msl_object_t mslo("my_render_msl_c") ;
                    ...
                    msl_obj_c = motor::shared( std::move( mslo ) ) ;
                    reconfigs.emplace_back( msl_obj_c ) ;
                }
            }
        }
        ...
        virtual void_t on_graphics( motor::application::app::graphics_data_in_t gdi ) noexcept 
        {
            ...
            if( animate2 )
            {
                my_layer = motor::math::interpolation<float_t>::linear( 1.0f, 20.0f, t2 ) ;
            }

            msl_obj_a->for_each( [&]( size_t const, motor::graphics::variable_set_mtr_t vs )
            {
                {
                ...
                {
                    auto * var = vs->data_variable<motor::math::vec2f_t>( "u_uv" ) ;
                    var->set( motor::math::vec2f_t(u, v) ) ;
                }
        ...
        { 
            ...
            if( ImGui::Begin( "Controls" ) )
            {
                ...
                ImGui::Checkbox( "Animate 2", &animate2 ) ; ImGui::SameLine() ;
                ImGui::SliderFloat( "Noise Layer", &my_layer, 1.0f, 20.0f ) ;

                ImGui::SliderFloat( "U", &u, 0.0f, 1.0f ) ;
        ...
        {
            motor::release( motor::move( msl_obj_a ) ) ;
            motor::release( motor::move( msl_obj_b ) ) ;
            motor::release( motor::move( msl_obj_c ) ) ;
